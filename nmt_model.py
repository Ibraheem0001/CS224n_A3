...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.target_vocab_projection = nn.Linear(in_features=hidden_size, out_features=len(self.vocab.tgt), bias=False) #(Linear Layer with no bias), called W_{vocab} in the PDF.
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        last_cell_cat = torch.cat((last_cell[0], last_cell[1]), axis=1) # Concatenate the forwards and backwards cell states
        init_decoder_cell = self.c_projection(last_cell_cat)
        dec_init_state = (init_decoder_hidden, init_decoder_cell) # the concatenated hidden and cell states serve as decoder hidden init (encoded info being used by decoder)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        Y = self.model_embeddings(target_padded, type="decoder")
        for Y_t in Y:
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        a_t = torch.bmm(torch.unsqueeze(alpha_t, dim=-1).transpose(2,1), enc_hiddens).squeeze()
